...
static int _ccv_dpm_scale_upto_modified(ccv_dense_matrix_t* a, ccv_dpm_mixture_model_t** _model, int count, int interval)
{
        return 0;
}
...
/*
a: weight matrix
_model: cotains several models (components) with several parts
return:
number to go up in feature pyramid of root filter that it matches size of matrix a minus next, minus next because feature pyramid needs this input
*/
{
	int c, i;
        //save minimum size of root filters over all models and all components, must be smaller than matrix a
	...
	int next = interval + 1;
	return (int)(log((double)ccv_min(hr, wr)) / log(scale)) - next; //(-next) because _ccv_ 
}

/*
output:
b: original: computed features, here: downsampled features, size matches fixed rows/cols of model component
input:
a: original: image, here: features at original resolution, size of a: CCV_DPM_WINDOW_SIZE*rows/cols, rows, cols: rows and cols of given model component
b_type: not used
s_bin: not used
size: original: cell size for hog features, here: downsampling factor
*/
static void ccv_hog_modified(ccv_dense_matrix_t* a, ccv_dense_matrix_t** b, int b_type, int sbin, int size)
{
        //*b = (ccv_dense_matrix*) malloc(sizeof(ccv_dense_matrix_t*));
        //*b = ccv_dense_matrix_new(a->rows, a->cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
	//memcpy((*b)->data.f32, a->data.f32, a->rows * a->step);
        int rows_new = (a->rows)/size, cols_new = (a->cols)/size; 
        ccv_resample(a, b, 0, rows_new, cols_new, CCV_INTER_AREA);
}

/*
modified version ignores scale_upto, interval or sets them effectively to 0
input:
a: features with original size
scale_upto  interval have no influence, only to match original interface
output: corresponds to _ccv_dpm_feature_pyramid with scale_upto=0 and interval=0
pyr[0]: features on original size
pyr[1]: features on half size
*/
static void _ccv_dpm_feature_pyramid_modified(ccv_dense_matrix_t* a, ccv_dense_matrix_t** pyr, int scale_upto, int interval)
{
	memset(pyr, 0, 2 * sizeof(ccv_dense_matrix_t*));
        //set pyr[0] with original features and par[1] with half resolution
        pyr[0] = ccv_dense_matrix_new(a->rows, a->cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
	memcpy(pyr[0]->data.f32, a->data.f32, a->rows * a->step);
        ccv_sample_down(pyr[0], &pyr[1], 0, 0, 0);
}


/*
calculate feature pyramide of image  with a resolution of interval steps within a scale factor of 2 
pyr[0 to intervall]: hog features with half windowsize and downscaling with a factor between 1 and 2
pyr[next to next+scale_upto]: hog features with full windowsize and downscaling with a factor 1 to scale^((scale_upto+next)
*/
static void _ccv_dpm_feature_pyramid(ccv_dense_matrix_t* a, ccv_dense_matrix_t** pyr, int scale_upto, int interval)
{
	...
	for (i = 1; i <= interval; i++)
        ...
        //ccv_sample_down downscale by a factor of 2
	...
	/* a more efficient way to generate up-scaled hog (using smaller size) */
        //computing hog of downscaled images and saving in pyr[0-interval] with full resolution (half window size)
	for (i = 0; i < next; i++)
	{
		hog = 0;
        ...
        //pyr[next] should not be freed because content of a should not be freed
	hog = 0;
	ccv_hog(pyr[next], &hog, 0, 9, CCV_DPM_WINDOW_SIZE);
	pyr[next] = hog;
        //computing hog of downscaled images and saving in pyr[next to (next+] with half resolution (full window size)
...
/*
input:
root_classifier: contains one component/root  with several part
hog: hog features for root filter
hog2x: hog features with double resolution for part filter
output:
_response: root filter response - deformation cost + part filter responses
part_feature: part filter response
dx: dx[i] is pointer on array of deformation dx of part i
dy: dy[i] is pointer on array of deformation dy of part i
*/
	...
	*_response = root_feature;
	...
	for (i = 0; i < root_classifier->count; i++)
	{
		...
		part_feature[i] = dx[i] = dy[i] = 0;
                //part->dx, ... : spring coefficients of parts
		...
		for (y = rwh; y < root_feature->rows - rwh_1; y++)
		{
			...
			for (x = rww; x < root_feature->cols - rww_1; x++)
			{
				int ix = ccv_clamp(x * 2 + offx, minx, maxx);
                                //-1 because distance transform is defined as min problem with -1 in contrast to Felzenswalb notation
...
static ccv_dpm_mixture_model_t* _ccv_dpm_model_copy(ccv_dpm_mixture_model_t* _model)
{
	...
	for (i = 0; i < model->count; i++)
	{
		...
		root->root.w = ccv_dense_matrix_new(_root->root.w->rows, _root->root.w->cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
		...
		{
			part[j].w = ccv_dense_matrix_new(_part[j].w->rows, _part[j].w->cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
...
static void _ccv_dpm_read_checkpoint(ccv_dpm_mixture_model_t* model, const char* dir)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		root_classifier[i].root.w = ccv_dense_matrix_new(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
		for (j = 0; j < rows * cols * CCV_NUMBER_CHANNELS; j++)
		...
		for (j = 0; j < root_classifier[i].count; j++)
		{
			...
			part_classifier[j].w = ccv_dense_matrix_new(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
			for (k = 0; k < rows * cols * CCV_NUMBER_CHANNELS; k++)
...
{
	assert(CCV_GET_CHANNEL(w->type) == CCV_NUMBER_CHANNELS && CCV_GET_DATA_TYPE(w->type) == CCV_32F);
	...
	for (i = 0; i < w->rows; i++)
	{
		for (j = 0; j < w->cols; j++)
		{
			for (k = 0; k < CCV_NUMBER_CHANNELS; k++)
			{
				double v = fabs(w_ptr[j * CCV_NUMBER_CHANNELS + k] - w_ptr[(w->cols - 1 - j) * CCV_NUMBER_CHANNELS + _ccv_dpm_sym_lut_modified[k]]);
				if (v > 0.002)
					PRINT(CCV_CLI_INFO, "symmetric violation at (%d, %d, %d), off by: %f\n", i, j, k, v);
			}
		}
		w_ptr += w->cols * CCV_NUMBER_CHANNELS;
	}
}

//struct to save parameters of one component
typedef struct {
	int id;                 //id identifies component in mixture model
	int count;
	float score;
	int x, y;
...
/*
output:
negex: ccv_array_t contains array of negnum feature_vector for each component, 
input: 
bgfiles: path to and name of bg file
bgnum: number of bg files
negum: number of negative examples that should be drawn out of bgfiles
components: number of model components
rows/cols?: size of bounding box for components 
*/
static void _ccv_dpm_collect_examples_randomly(gsl_rng* rng, ccv_array_t** negex, char** bgfiles, int bgnum, int negnum, int components, int* rows, int* cols, int grayscale)
{
	...
	//determine maximal width and height of bounding boxes of all components
	...
	{
                //p: number of neg examples per bg image
		...
		for (i = 0; i < bgnum; i++)
			if (gsl_rng_uniform(rng) < p)
			{
				...
				//ccv_read(bgfiles[i], &image, (grayscale ? CCV_IO_GRAY : 0) | CCV_IO_ANY_FILE);
                                ccv_read_modified(bgfiles[i], "data", &image, CCV_NUMBER_CHANNELS);
				assert(image != 0);
				...
				for (j = 0; j < components; j++)
				{
					...
					ccv_hog_modified(slice, &hog, 0, 9, CCV_DPM_WINDOW_SIZE);
					...
					assert(hog->rows == rows[j] && hog->cols == cols[j] && CCV_GET_CHANNEL(hog->type) == CCV_NUMBER_CHANNELS && CCV_GET_DATA_TYPE(hog->type) == CCV_32F);
					vector.root.w = hog;
...
/*
produce pointer to feature vectors containing features of root in replaced and rescaled bounding box
posfiles: path and name to positive images
bboxes: pointer to array of bboxes
posnum: number of positive examples
id: component?
rows/cols: size of root filter in hog cells?
return: pointer to array of posnum ccv_feature_vectors containing hog features within modified boundingbox in order to match fixed aspect ratio and size 
aspect ratio and size deduced by rows and cols, reshaped bbox is also recentered
*/
static ccv_array_t* _ccv_dpm_summon_examples_by_rectangle(char** posfiles, ccv_rect_t* bboxes, int posnum, int id, int rows, int cols, int grayscale)
{
	...
	for (i = 0; i < posnum; i++)
	{
                //modify bbox that area is unchanged but aspect ratio is fixed by rows and cols 
		...
		//ccv_read(posfiles[i], &image, (grayscale ? CCV_IO_GRAY : 0) | CCV_IO_ANY_FILE);
                ccv_read_modified(posfiles[i], "data", &image, CCV_NUMBER_CHANNELS);
		assert(image != 0);
		...
		ccv_dense_matrix_t* resize = 0;
                //CCV_INTER_AREA for downsampling 
		ccv_resample(slice, &resize, 0, rows * CCV_DPM_WINDOW_SIZE, cols * CCV_DPM_WINDOW_SIZE, CCV_INTER_AREA);
		ccv_matrix_free(slice);
		ccv_dense_matrix_t* hog = 0;
		ccv_hog_modified(resize, &hog, 0, 9, CCV_DPM_WINDOW_SIZE);
		ccv_matrix_free(resize);
		ccv_make_matrix_mutable(hog);
		assert(hog->rows == rows && hog->cols == cols && CCV_GET_CHANNEL(hog->type) == CCV_NUMBER_CHANNELS && CCV_GET_DATA_TYPE(hog->type) == CCV_32F);
		vector.root.w = hog;
		ccv_array_push(posv, &vector);
...
/*
Initialize root filter with linear SVM, which is trained on positive example features within bounding boxes for model component i and negative example features within bounding box for model component i 
root_classifier: root_classifier->root.w is set to weights of trained linear SVM, root_classifier->beta is set to bias of linear SVM weights
label: component id ?
cnum: number of posex with poslabel==label
poslabel: pointer to labels of posex, labels identify component?
posex: array of positive ex., containing feature_vector with root filter
negex: array of negative ex
C: constant for linear SVM
symmetric: if =1 training size is doubled by taking left and right side separately 
*/
static void _ccv_dpm_initialize_root_classifier(gsl_rng* rng, ccv_dpm_root_classifier_t* root_classifier, int label, int cnum, int* poslabels, ccv_array_t* posex, int* neglabels, ccv_array_t* negex, double C, int symmetric, int grayscale)
{
	...
	PRINT(CCV_CLI_INFO, " - creating initial model %d at %dx%d\n", label + 1, cols, rows);
        //struct problem saves features and labels in data format for linear SVM in liblinear
	struct problem prob;
	prob.n = symmetric ? CCV_NUMBER_CHANNELS * cols2c * rows + 1 : CCV_NUMBER_CHANNELS * cols * rows + 1;
	...
	l = 0;
        //write root weights of positive examples as features for SVM in prob.x, mark positive labels with a one in prob.y
	for (i = 0; i < posex->rnum; i++)
		if (poslabels[i] == label)
		{
			...
			if (symmetric)
			{
				features = (struct feature_node*)malloc(sizeof(struct feature_node) * (CCV_NUMBER_CHANNELS * cols2c * rows + 2));
				...
				for (y = 0; y < rows; y++)
				{
					for (x = 0; x < cols2c; x++)
						for (k = 0; k < CCV_NUMBER_CHANNELS; k++)
						{
							...
							features[j].value = hptr[x * CCV_NUMBER_CHANNELS + k];
							++j;
						}
					hptr += hog->cols * CCV_NUMBER_CHANNELS;
				}
				features[j].index = j + 1;
				...
				for (y = 0; y < rows; y++)
				{
					for (x = 0; x < cols2c; x++)
						...
						{
							features[j].index = j + 1;
							features[j].value = hptr[(cols - 1 - x) * CCV_NUMBER_CHANNELS + _ccv_dpm_sym_lut_modified[k]];
							++j;
						}
					hptr += hog->cols * CCV_NUMBER_CHANNELS;
				}
				...
				features = (struct feature_node*)malloc(sizeof(struct feature_node) * (CCV_NUMBER_CHANNELS * cols * rows + 2));
				for (j = 0; j < rows * cols * CCV_NUMBER_CHANNELS; j++)
				{
					features[j].index = j + 1;
					features[j].value = hog->data.f32[j];
				}
				features[CCV_NUMBER_CHANNELS * rows * cols].index = CCV_NUMBER_CHANNELS * rows * cols + 1;
				features[CCV_NUMBER_CHANNELS * rows * cols].value = prob.bias;
				features[CCV_NUMBER_CHANNELS * rows * cols + 1].index = -1;
				prob.x[l] = features;
				prob.y[l] = 1;
				++l;
			}
        ...
        //save negative root weights as features in prob.x and mark prob.y with -1 for negative ex
	for (i = 0; i < negex->rnum; i++)
		if (neglabels[i] == label)
		{
			...
			if (symmetric)
			{
				...
				features = (struct feature_node*)malloc(sizeof(struct feature_node) * (CCV_NUMBER_CHANNELS * cols * rows + 2));
				for (j = 0; j < CCV_NUMBER_CHANNELS * rows * cols; j++)
				{
					features[j].index = j + 1;
					features[j].value = hog->data.f32[j];
				}
	...
	prob.l = l;
        //use weights of linear SVM for initializing root filter
	...
	if (symmetric)
	{
		...
		for (y = 0; y < rows; y++)
		{
			for (x = 0; x < cols2c; x++)
				for (k = 0; k < CCV_NUMBER_CHANNELS; k++)
					wptr[(cols - 1 - x) * CCV_NUMBER_CHANNELS + _ccv_dpm_sym_lut_modified[k]] = wptr[x * CCV_NUMBER_CHANNELS + k] = linear->w[(y * cols2c + x) * CCV_NUMBER_CHANNELS + k];
			wptr += cols * CCV_NUMBER_CHANNELS;
		}
		// since for symmetric, lsvm only computed half features, to compensate that, we doubled the constant.
		root_classifier->beta = linear->w[CCV_NUMBER_CHANNELS * rows * cols2c] * 2.0;
	} else {
		for (j = 0; j < CCV_NUMBER_CHANNELS * rows * cols; j++)
			root_classifier->root.w->data.f32[j] = linear->w[j];
		root_classifier->beta = linear->w[CCV_NUMBER_CHANNELS * rows * cols];
	}
	...
	for (j = 0; j < prob.l; j++)
		free(prob.x[j]);
	free(prob.x);
        ccv_write_modified(root_classifier->root.w, "../DPM/samples/filters/root_initialized.h5", "data", CCV_NUMBER_CHANNELS);
	ccv_make_matrix_immutable(root_classifier->root.w);
}

/*
root_classifier: root_classifier->part are initialized
...
static void _ccv_dpm_initialize_part_classifiers(ccv_dpm_root_classifier_t* root_classifier, int parts, int symmetric)
{
	...
	for (i = 0; i < parts;)
	{
                //search for positions and size of part filters
...
#define slice_and_update_if_needed(y, x, l, n, s) \
		...
		part_classifier->z = 1;
		part_classifier->w = 0;
                //initialize part filter with slice of root filter with double resolution
		...
		float* w_ptr = (float*)ccv_get_dense_matrix_cell_by(CCV_32F | CCV_NUMBER_CHANNELS, w, dy, dx, 0);
		for (y = 0; y < dh; y++)
		{
			for (x = 0; x < dw * CCV_NUMBER_CHANNELS; x++)
				w_ptr[x] = 0;
			w_ptr += w->cols * CCV_NUMBER_CHANNELS;
		}
		i++;
...
/*
write sizes of root_classifier in feature_vector, but not weights?
*/ 
static void _ccv_dpm_initialize_feature_vector_on_pattern(ccv_dpm_feature_vector_t* vector, ccv_dpm_root_classifier_t* root, int id)
{
	...
	vector->root.w = ccv_dense_matrix_new(root->root.w->rows, root->root.w->cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
	for (i = 0; i < vector->count; i++)
	{
		vector->part[i].x = root->part[i].x;
		vector->part[i].y = root->part[i].y;
		vector->part[i].z = root->part[i].z;
		vector->part[i].w = ccv_dense_matrix_new(root->part[i].w->rows, root->part[i].w->cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
	}
}
...
/*
Calculate score during training process
model: contains parameters that should be learned
v: contains features extracted out of training examples
*/
...
/*
output:
v: write image features in root region at location x,y and part regions at optimal position in feature vector v 
input:
score: saved in v->score
x,y: position of root filter
pyr: hog features out of feature pyramide at one scale
detail: double resolution hog features for part filters?
dx[i], dy[i]: array containing deformation out of distance transform of part i 
*/
static void _ccv_dpm_collect_feature_vector(ccv_dpm_feature_vector_t* v, float score, int x, int y, ccv_dense_matrix_t* pyr, ccv_dense_matrix_t* detail, ccv_dense_matrix_t** dx, ccv_dense_matrix_t** dy)
{
	...
	float* w_ptr = v->root.w->data.f32;
        //copy data of feature pyramid into root.w of feature vector
...
/*
description: labeled bbox is varied in scale and position around given position and scale, allowed amount of variation given by overlap, to find highest score position and scale, return feature vector with highest score and setted weights
scales down image/hog (feat pyr) to match bbox with size of root_filter, find scales and positions of new bbox that size and position overlap with given bbox and determine position and scale with highest score and save features in v, position and scale are choosen to match given bbox
image: image with bbox as positive trainingsexample
model: model with fixed parameters
bbox: bbox of positive example
overlap:1. root filter is upsampled with factor scale as long as its area divided by bbox rectangle is bigger than overlap, !!!this overlap need to be set to 0 for fixed scale!!!
        2. for finding highest score of positive example, area of intersection of replaced bbox and given rectangle divided by area of bigger rectangle need to be larger than overlap !!!this overlap must still exist for fixed scale!!!  
return:
v: collected features of positive example treating position of bbox as roughly given 
modifications for fix scale:
1. overlap (see overlap paramter)
2. scale_upto is not needed to be computed
*/
static ccv_dpm_feature_vector_t* _ccv_dpm_collect_best(ccv_dense_matrix_t* image, ccv_dpm_mixture_model_t* model, ccv_rect_t bbox, double overlap, ccv_dpm_param_t params)
{
	...
	int next = 1;//params.interval + 1;
        //scale_upto: steps in feature pyramide to go up that root filter in model matches image size
	int scale_upto = 0;//_ccv_dpm_scale_upto(image, &model, 1, params.interval);
	if (scale_upto < 0)
		return 0;
	ccv_dense_matrix_t** pyr = (ccv_dense_matrix_t**)alloca((scale_upto + next * 2) * sizeof(ccv_dense_matrix_t*));
	_ccv_dpm_feature_pyramid_modified(image, pyr, scale_upto, params.interval);
	float best = -FLT_MAX;
	...
	for (i = 0; i < model->count; i++)
	{
		...
		double scale_x = 1.0;
		double scale_y = 1.0;
                //in the following for loop the second argument should be set to j<next+1
		for (j = next; j < scale_upto + next * 2; j++)
		{
			...
			/* not used because scale is fixed
				...
				ccv_max((double)(bbox.width * bbox.height), (double)(size.width * size.height)) < overlap)
			{
				...
				continue;
                        ...
                        */
                        //response of root filter, deformation and part filter saved in root_feature
			ccv_dense_matrix_t* root_feature = 0;
                        //CCV_DPM_PART_MAX by default 10
                        //response of part filter saved in part_feature
			ccv_dense_matrix_t* part_feature[CCV_DPM_PART_MAX];
                        //distance transforms of part filters saved in dx, dy
			...
			int rwh_1 = root_classifier->root.w->rows / 2, rww_1 = root_classifier->root.w->cols / 2;
			float* f_ptr = (float*)ccv_get_dense_matrix_cell_by(CCV_32F | CCV_C1, root_feature, rwh, 0, 0);
                        //sliding window approach of root filter 
			for (y = rwh; y < root_feature->rows - rwh_1; y++)
			{
				for (x = rww; x < root_feature->cols - rww_1; x++)
				{
                                        /*reconstruct bbox out of root filter location and size by rescaling, rescaled bbox (rect) need to have minimum
                                        area overlap with bbox of positive example
                                        */ 
						...
						(double)ccv_max(rect.width * rect.height, bbox.width * bbox.height) >= overlap && f_ptr[x] > best)
					{
						...
						if (v->id != i)
						{
							...
							_ccv_dpm_initialize_feature_vector_on_pattern(v, root_classifier, i);
                                                ...
                                                //pyr[j-next]: features at double resolution
...
/*
collect first 64 feature vectors of negative examples, that have a score greater than threshold, could contain less than 64 feature vectors
return:
av: pointer on array of pointers to features vectors with score greater than threshold, for each model component same amount (int enough) 
*/
static ccv_array_t* _ccv_dpm_collect_all(gsl_rng* rng, ccv_dense_matrix_t* image, ccv_dpm_mixture_model_t* model, ccv_dpm_param_t params, float threshold)
{
	...
	ccv_dense_matrix_t** pyr = (ccv_dense_matrix_t**)alloca((scale_upto + next * 2) * sizeof(ccv_dense_matrix_t*));
	_ccv_dpm_feature_pyramid_modified(image, pyr, scale_upto, params.interval);
        //array of 64 feature vectors to store 
...
/*
collecting upto params.negative_cache_size negative feature vectors from background, maximal 64 examples out of one negative image
return: av feature vectors with negative examples
*/
static void _ccv_dpm_collect_from_background(ccv_array_t* av, gsl_rng* rng, char** bgfiles, int bgnum, ccv_dpm_mixture_model_t* model, ccv_dpm_new_param_t params, float threshold)
{
	...
	for (i = 0; i < bgnum; i++)
	{
		...
		ccv_dense_matrix_t* image = 0;
		//ccv_read_modified(bgfiles[order[i]], &image, (params.grayscale ? CCV_IO_GRAY : 0) | CCV_IO_ANY_FILE);
                ccv_read_modified(bgfiles[order[i]], "data", &image, CCV_NUMBER_CHANNELS);
...
/*
function to learn a multilinear model for each component separately for prediction of bounding boxes
*/
...
{
	int i, j, k, c;
        //save best feature vectors of each positive example in posv
	ccv_dpm_feature_vector_t** posv = (ccv_dpm_feature_vector_t**)ccmalloc(sizeof(ccv_dpm_feature_vector_t*) * posnum);
        //count number of examples for each model component
	...
	{
		ccv_dpm_root_classifier_t* root_classifier = model->root + i;
                //allocate matrix with num_per_model rows and 2*#parts*1 columns, X is feature matrix to learn for bbox prediction
		...
		for (j = 0; j < posnum; j++)
		{
			...
			if (v && v->id == i)
			{
                ...
                //create linear classfier object for predicting bounding boxes
		gsl_multifit_linear_workspace* workspace = gsl_multifit_linear_alloc(num_per_model[i], root_classifier->count * 2 + 1);
		double chisq;
                //save model parameters in root.alpha and root->part.alpha
		for (j = 0; j < 3; j++)
		{
...
/*
weights of model component i are reduced by regz*weight, i.e. root filter, part filters and spring constants
*/
...
/*
calculates beta = beta + alpha*n*C*y*phi, with additional regularization step one gradient descent step is calculated
model: model containing all paramaters beta
v: feature vector of one trainings example
y: label of trainings example v
alpha: learning rate
Cn: C*n, n: number of trainingsexamples
symmeteric: wether filter are symmetric, if symmetric to gradient descent steps are done 
*/
		...
		for (i = 0; i < v->root.w->rows; i++)
		{
			for (j = 0; j < v->root.w->cols; j++)
				for (c = 0; c < ch; c++)
				{
					...
					wptr[j * ch + c] += alpha * y * Cn * vptr[(v->root.w->cols - 1 - j) * ch + _ccv_dpm_sym_lut_modified[c]];
				}
			...
			if (part_classifier->counterpart == -1)
			{
				...
				for (i = 0; i < part_vector->w->rows; i++)
				{
					for (j = 0; j < part_vector->w->cols; j++)
						...
						{
							wptr[j * ch + c] += alpha * y * Cn * vptr[j * ch + c];
							wptr[j * ch + c] += alpha * y * Cn * vptr[(part_vector->w->cols - 1 - j) * ch + _ccv_dpm_sym_lut_modified[c]];
						}
...
static ccv_dpm_feature_vector_t* _ccv_dpm_read_feature_vector(FILE* r)
{
	...
	ccv_dpm_feature_vector_t* v = (ccv_dpm_feature_vector_t*)ccmalloc(sizeof(ccv_dpm_feature_vector_t));
	v->id = id;
	v->root.w = ccv_dense_matrix_new(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
	for (j = 0; j < rows * cols * CCV_NUMBER_CHANNELS; j++)
	...
	for (j = 0; j < v->count; j++)
	{
		...
		fscanf(r, "%d %d", &rows, &cols);
		part_classifier->w = ccv_dense_matrix_new(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
		for (k = 0; k < rows * cols * CCV_NUMBER_CHANNELS; k++)
...
#define REGQ (100)
/*
relabel: number of relabeling model components, relabel->stochastic gradient descent -> relabel -> ...
balance: how are positive and negative examples weighted (for reducing false positive rate)
alpha_ratio: alpha *=alpha_ratio after each iteration
iterations: number of stochastic gradient descent runs, one run is with all positive and negative examples
*/
static ccv_dpm_mixture_model_t* _ccv_dpm_optimize_root_mixture_model(gsl_rng* rng, ccv_dpm_mixture_model_t* model, ccv_array_t** posex, ccv_array_t** negex, int relabels, double balance, double C, double previous_alpha, double alpha_ratio, int iterations, int symmetric)
{
	...
	{
                //saves number of positive examples of model i
		...
		memset(neg_prog, 0, sizeof(int) * model->count);
                //negative examples are asigned randomly to model component
		for (i = 0; i < negnum; i++)
		{
...
{    
        ...
        for(int i=0; i<3; i++)
        {
        //char file_name[128] = "../samples/fly/filters/part_filter_";
        //int length = sizeof("../samples/fly/filters/part_filter_");
        //file_name[length] = i+'0';
        //strcat(file_name, str_i);
        //strcat(file_name, (char)i);
        //file_name[length+1] = "_learned.h5";
        PRINT(CCV_CLI_INFO, file_name[i]);
        }   
        ccv_dense_matrix_t *res = 0;
        //test read in 
        ccv_read_modified(posfiles[0], "data", &res, CCV_NUMBER_CHANNELS);
	ccv_dense_matrix_t** pyr = (ccv_dense_matrix_t**)alloca(2 * sizeof(ccv_dense_matrix_t*));
        //test feature pyramid
        _ccv_dpm_feature_pyramid_modified(res, pyr, 0, 0);
        PRINT(CCV_CLI_INFO, "pyr[0]->cols: %d, pyr[0]->rows: %d, pyr[1]->cols: %d, pyr[1]->rows: %d", pyr[0]->cols, pyr[0]->rows, pyr[1]->cols, pyr[1]->rows);
        //int equal = ccv_is_equal_modified(res, pyr[0], CCV_NUMBER_CHANNELS); 
        //test if downsampled image works 
        int row = 100, col = 100, ch = 3;
        PRINT(CCV_CLI_INFO, "feature big %f, feature small %f", res->data.f32[1024*row*4+col*4+ch], pyr[1]->data.f32[512*(row/2)*4+(col/2)*4+ch]  );
        ccv_matrix_free(pyr[0]);
        ccv_matrix_free(pyr[1]);
        ccv_dense_matrix_t *hog = 0;
        ccv_hog_modified(res, &hog, 0, 0, CCV_DPM_WINDOW_SIZE);
        PRINT(CCV_CLI_INFO,"rows: %d, cols: %d", hog->cols, hog->rows);
        //PRINT(CCV_CLI_INFO, "is datatype CCV_32F:%d", CCV_GET_DATA_TYPE((res[0])->type) == CCV_32F);
        //ccv_dense_matrix_t *dptr = *res;
        //PRINT(CCV_CLI_INFO, "reached %d", CCV_GET_DATA_TYPE(res->type) == CCV_32F);
        ccv_matrix_free(hog);
        ccv_matrix_free(res);
        PRINT(CCV_CLI_INFO, "reached matrix_free");
        //*res = 0;
        //free(res);
        return 0;
}
...
{       
	...
	sprintf(initcheckpoint, "%s/init.model", dir);
        //sort positive examples according to their aspect ratio
	...
	int outnum = posnum, innum = 0;
        //positive examples are sorted by their aspect ratio, the mean and variance of aspect ratio is printed 
	...
	int* cols = (int*)alloca(sizeof(int) * params.components);
        //save to each positive example model assignment according to aspect ratio in poslabels and in rows and cols size of root filter with fix small area
	for (i = 0; i < params.components; i++)
	{
		double aspect = 0;
		for (j = innum; j < innum + mnum[i]; j++)
	...
	if (corrupted)
	{
                //save features of pos and neg examples within bboxes for different components(posex, negex), label them according to aspect ratio (poslabels) 
                //and use SVM to initialize root filter
		...
		{
			ccv_dpm_root_classifier_t* root_classifier = model->root + i;
			root_classifier->root.w = ccv_dense_matrix_new(rows[i], cols[i], CCV_32F | CCV_NUMBER_CHANNELS, 0, 0);
	...
	for (; c < params.relabels; c++)
	{
		...
		for (; d < params.data_minings; d++)
		{
			...
			if (negv)
			{
                                //if negv exist already, do datamining: retain hard examples, refill later thrown easy examples by new one
				ccv_array_t* av = ccv_array_new(sizeof(ccv_dpm_feature_vector_t*), 64, 0);
			...
			if (negv->rnum < params.negative_cache_size)
                                //collect only upto params.negative_cache_size negative examples, bgnum is parameter to limit maximal used bgfiles
			...
			for (t = 0; t < params.iterations; t++)
			{
				for (p = 0; p < model->count; p++)
				{
					...
					for (i = 0; i < posnum + negv->rnum; i++)
					{
						...
						++l;
                                                //after REGQ gradient descent steps regularize
						if (l % REGQ == REGQ - 1)
						...
						if (l % MINI_BATCH == MINI_BATCH - 1)
						{
							...
							_model = _ccv_dpm_model_copy(model);
                                        ...
                                        //why regularization after iteration step
			...
			PRINT(CCV_CLI_INFO, " - threshold breakdown by percentile");
                        //print scores in percentile_breakdown steps
	...
	if (negv)
	{
        ...
        ccv_write_modified(model->root->root.w, "../DPM/samples/filters/root_filter_learned.h5", "data", CCV_NUMBER_CHANNELS);
        char* file_name[3];
        file_name[0] = "../DPM/samples/filters/part_filter_1_learned.h5";
        file_name[1] = "../DPM/samples/filters/part_filter_2_learned.h5";
        file_name[2] = "../DPM/samples/filters/part_filter_3_learned.h5";
...
void test_detect(ccv_dense_matrix_t* a, ccv_dpm_mixture_model_t** _model, int count, ccv_dpm_param_t params, char* filename)
{
	...
	_ccv_dpm_feature_pyramid_modified(a, pyr, 0, 0);
        ...
        ccv_dpm_root_classifier_t* root = model->root;
        ccv_dense_matrix_t* root_feature = 0;
        ccv_dense_matrix_t* part_feature[CCV_DPM_PART_MAX];
        ccv_dense_matrix_t* dx[CCV_DPM_PART_MAX];
        ccv_dense_matrix_t* dy[CCV_DPM_PART_MAX];
...
ccv_array_t* ccv_dpm_detect_objects(ccv_dense_matrix_t* a, ccv_dpm_mixture_model_t** _model, int count, ccv_dpm_param_t params)
{
	...
	double scale = pow(2.0, 1.0 / (params.interval + 1.0));
	int next = 1; //= params.interval + 1;
	int scale_upto = 0; // _ccv_dpm_scale_upto(a, _model, count, params.interval);
	if (scale_upto < 0) // image is too small to be interesting
		return 0;
	ccv_dense_matrix_t** pyr = (ccv_dense_matrix_t**)alloca((scale_upto + next * 2) * sizeof(ccv_dense_matrix_t*));
	_ccv_dpm_feature_pyramid_modified(a, pyr, scale_upto, params.interval);
	ccv_array_t* idx_seq;
	...
	for (c = 0; c < count; c++)
	{
		...
		double scale_x = 1.0;
		double scale_y = 1.0;
                //collect bbinfo in seq of all pixels with score greater than threshold in params
					...
					for (x = rww; x < root_feature->cols - rww_1; x++)
						if (f_ptr[x] + root->beta > params.threshold)
						{
							...
							comp.classification.id = c + 1;
                                                        //save score at current location
							...
							for (k = 0; k < root->count; k++)
							{
								...
								int ix = ccv_clamp(x * 2 + offx, pww, part_feature[k]->cols - part->w->cols + pww);
                                                                //get deformation at current location 
		...
		if (params.min_neighbors == 0)
		{
			...
			// group retrieved rectangles in order to filter out noise
                        // assign each location with score>threshold an id 
...
ccv_dpm_mixture_model_t* ccv_dpm_read_mixture_model(const char* directory)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		fscanf(r, "%d %d", &rows, &cols);
		fscanf(r, "%f %f %f %f", &root_classifier[i].beta, &root_classifier[i].alpha[0], &root_classifier[i].alpha[1], &root_classifier[i].alpha[2]);
		root_classifier[i].root.w = ccv_dense_matrix_new(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS, ccmalloc(ccv_compute_dense_matrix_size(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS)), 0);
		size += ccv_compute_dense_matrix_size(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS);
		for (j = 0; j < rows * cols * CCV_NUMBER_CHANNELS; j++)
			fscanf(r, "%f", &root_classifier[i].root.w->data.f32[j]);
		ccv_make_matrix_immutable(root_classifier[i].root.w);
		fscanf(r, "%d", &root_classifier[i].count);
		...
		for (j = 0; j < root_classifier[i].count; j++)
		{
			...
			fscanf(r, "%f %f %f %f %f %f", &part_classifier[j].alpha[0], &part_classifier[j].alpha[1], &part_classifier[j].alpha[2], &part_classifier[j].alpha[3], &part_classifier[j].alpha[4], &part_classifier[j].alpha[5]);
			fscanf(r, "%d %d %d", &rows, &cols, &part_classifier[j].counterpart);
			part_classifier[j].w = ccv_dense_matrix_new(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS, ccmalloc(ccv_compute_dense_matrix_size(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS)), 0);
			size += ccv_compute_dense_matrix_size(rows, cols, CCV_32F | CCV_NUMBER_CHANNELS);
			for (k = 0; k < rows * cols * CCV_NUMBER_CHANNELS; k++)
				fscanf(r, "%f", &part_classifier[j].w->data.f32[k]);
			ccv_make_matrix_immutable(part_classifier[j].w);
